 ...
 *	Use a for loop to input the elements of the array. As you input each element, add it to the sum variable.
 *	Calculate the average using the formula average = sum / n.
 *	Use another for loop to iterate through the array. For each element, check if it is less than the average. If the condition holds, print that element.
 *
 */
